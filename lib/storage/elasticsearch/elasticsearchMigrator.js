...
ElasticsearchMigrator.prototype._putMapping = function (opts) {
  ...
  var promises = _.map(opts.blueprints, function(blueprint) {
    var mapping = {};
    ...
    _.each(blueprint.columns, function(column, columnKey) {
      mapping[blueprint.elasticsearch.type].properties[columnKey] = self['_define' + help.capitalizeFirstLetter(column.type).replace('[]', '')](column);

      // if extra options are given for elasticsearch, use those instead
      if (column.elasticsearch) {
        mapping[blueprint.elasticsearch.type].properties[columnKey] = column.elasticsearch;
      }
    });
