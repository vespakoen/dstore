    ...
    .then(function (projectVersion) {
      if (_.isNumber(projectVersion) && projectVersion > 0) {
        ...
        return BBPromise.all(_.map(versions, function (version) {
          var database = projectId + 'v' + version;
          
          return opts.client.queryAsync("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '" + database + "'")
            .then(function () {
              return opts.client.queryAsync('DROP DATABASE ' + database);
            })
            .then(function () {
