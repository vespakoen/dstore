 ...
 */
function LevelRepository(serializer, adapter, blueprintAdapter, itemTransformer, validator) {
...
LevelRepository.attach = function (app) {
  return BBPromise.join(
    ...
    app.get('validator')
  ).spread(function (serializer, adapter, blueprintAdapter, itemTransformer, validator) {
    return new LevelRepository(serializer, adapter, blueprintAdapter, itemTransformer, validator);
  });
};
 ...
 * @return {BBPromise}
 */
LevelRepository.prototype.putItem = function (namespace, blueprintKey, id, item) {
  var self = this;
  ...
  return this._validatePutItem(namespace, blueprintKey, id, item)
    ...
    .then(function(item) {
      ...
      var table = itemByTypeDb.sublevel(blueprintKey);
      var itemByTypeAndIdDb = client.sublevel('item-by-type-and-id', bytewiseCodec);

      return BBPromise.promisify(typeByIdDb.put)(id, blueprintKey)
        .then(function () {
        ...
        .then(function () {
          return BBPromise.promisify(itemByTypeAndIdDb.put)([blueprintKey, id], value);
        })
    });
};
 ...
 * @return {BBPromise}
 */
LevelRepository.prototype.delItem = function (namespace, blueprintKey, id) {
  var self = this;
