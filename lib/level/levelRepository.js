...
LevelRepository.prototype.putItem = function (namespace, schemaKey, item) {
  ...
  return this._validatePutItem(namespace, schemaKey, item)
    ...
    .then(function(item) {
      ...
      return Promise.join(
        Promise.promisify(typeByIdDb.put)(item.id, schemaKey),
        Promise.promisify(itemByIdDb.put)(item.id, value),
        Promise.promisify(table.put)(item.id, value),
        Promise.promisify(itemByTypeAndIdDb.put)([schemaKey, item.id], value)
      )
      .then(function () {
        console.log('jeej');
      })
