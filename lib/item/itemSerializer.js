 ...
 */
function ItemSerializer(blueprintAdapter) {
 ...
 */
ItemSerializer.prototype._serializeItem = function (namespace, blueprintKey, item) {
  var self = this;

  var blueprintClient = this.blueprintAdapter.getClient(namespace);
  return blueprintClient.getSnapshot(item.snapshot_version).then(function(blueprints) {
    var column;
    var blueprint = blueprints[blueprintKey];
