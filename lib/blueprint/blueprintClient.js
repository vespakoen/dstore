...
BlueprintClient.prototype.forEveryVersionOfBlueprint = function(blueprintKey, version, cb) {
  var self = this;

  var allVersions = self.getVersionsForBlueprint(blueprintKey, version);
  return BBPromise.all(_.map(allVersions, function(version) {
    return cb(version);
  }));
};

BlueprintClient.prototype.getVersionsForBlueprint = function(blueprintKey, version) {
  var i;
  var self = this;
  var currentBlueprintKey = blueprintKey;
