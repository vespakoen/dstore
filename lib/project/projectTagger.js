...
ProjectTagger.prototype.tagProject = function(projectId) {
  ...
  return this.client.getProjectVersion(projectId)
    .then(function (projectVersion) {
      ...
      opts.newVersion = ((projectVersion === 'current' ? 0 : projectVersion) + 1);
    ...
    .then(function (blueprints) {
      ...
      return self.publisher.publish('migrate', opts.command);
    })
    .then(function () {
    ...
    .then(function () {
      if (opts.command.project_version > 1) {
        return self.publisher.publish('sync', opts.command);
      }
    })
    .then(function () {
...
ProjectTagger.prototype._getLatestAndCurrentBlueprints = function(opts) {
  ...
  return BBPromise.join(
    opts.projectVersion === 'current' ? {} : this.blueprintFacade.getAllBlueprints(opts.projectId, opts.projectVersion),
