 ...
 * @return {BBPromise}
 */
ElasticsearchMigrator.prototype.migrate = function(namespace, snapshotVersion, blueprints) {
  var self = this;
  var opts = {
...
ElasticsearchMigrator.prototype._putMapping = function (opts) {
  ...
  var promises = _.map(opts.blueprints, function(blueprint, blueprintKey) {
    var mapping = {};

    mapping[blueprintKey] = {
      _id: {path: 'id'},
      properties: {}
    };

    _.each(blueprint.columns, function(column, columnKey) {
      mapping[blueprintKey].properties[columnKey] = self['_define' + help.capitalizeFirstLetter(column.type).replace('[]', '')](column);

      // if extra options are given for elasticsearch, use those instead
      if (column.elasticsearch) {
        mapping[blueprintKey].properties[columnKey] = column.elasticsearch;
      }
    });
