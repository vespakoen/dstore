 ...
 * @return {BBPromise}
 */
ElasticsearchMigrator.prototype.migrate = function(namespace, snapshotVersion) {
  var self = this;
  var opts = {
...
ElasticsearchMigrator.prototype._putMapping = function (opts) {
  ...
  var promises = _.map(opts.schemas, function(schema, schemaKey) {
    ...
    _.each(schema.columns, function(column, columnKey) {
      self['_define' + help.capitalizeFirstLetter(column.type).replace('[]', '')](mapping, schemaKey, column, columnKey);
 ...
 * @param  {String} columnKey
 */
ElasticsearchMigrator.prototype._defineJson = function (mapping, schemaKey, column, columnKey) {
  mapping[schemaKey].properties[columnKey] = {
    type: 'object',
    enabled: false
  };
