...
PostgresqlMigrator.prototype._createDatabase = function(opts) {
  return pg.connectAsync(opts.connectionString).spread(function(client, done) {
    return client.queryAsync('CREATE DATABASE ' + opts.database + ' TEMPLATE=template_postgis').then(function() {
      console.log('database created');
    }).catch(function(err) {
      console.log('database already exists', err);
    });
  });
};

PostgresqlMigrator.prototype._getDatabaseClient = function(opts) {
  console.log('get database client');
...
PostgresqlMigrator.prototype._createTables = function(opts) {
  ...
  return Promise.all(promises).then(function() {
    console.log('done migrating postgres');
  });
};
